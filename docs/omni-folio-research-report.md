...
G4R는 공식 [`0B` WebSocket sample](https://github.com/Kiwoom-Securities/Kiwoom-REST-API/blob/9180debf7aea0074715dd8f7a15af432afbfc403/examples/%EA%B5%AD%EB%82%B4%EC%A3%BC%EC%8B%9D/%EC%8B%A4%EC%8B%9C%EA%B0%84%EC%8B%9C%EC%84%B8/subscribe_domestic_stock_trade_async.py)의 등록 shape와 FID `10`/`20`만 credential-free parser로 고정한다. 날짜 없는 체결 clock을 수신 날짜와 결합하지 않고 FID `9081`의 값도 추측하지 않아 venue를 생략한다. 실제 connection/reconnect, exchange mapping과 freshness는 credentialed mock 관찰 뒤에 추가한다.
