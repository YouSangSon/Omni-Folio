...
- 현재 G4H 증거는 credential-free 합성 Kiwoom complete snapshot을 insert-only SQLite에 원자 저장하고 저장 시점 ledger revision의 KRX/KRW 종목 수량 차이를 exact decimal로 보존하는 데 한정한다. 실패·불완전·충돌 입력은 이전 known-good를 바꾸지 않으며 backup v3가 broker record digest/count를 검증한다. 실제 broker request, scheduling, freshness/timezone, 현금·평가금액 reconciliation, public API/UI 또는 risk authority는 증명하지 않는다.
