...
현재 SQLite schema v5 원장은 `DEPOSIT`, `WITHDRAWAL`, `BUY`, `SELL`, `DIVIDEND`, `FEE`, `TAX`, `SPLIT`을 exact decimal로 replay한다. 출금·수수료·세금은 음수, 입금·현금배당은 양수, 분할은 0 cash impact와 양수 비율만 허용하며 열린 FIFO lot의 수량만 조정하고 원가는 보존한다. 열린 보유가 없는 분할과 잘못된 부호는 원자 rollback한다. 이는 FX, 정정 event, 배당 재투자, 국가별 세금 분류나 실제 broker reconciliation을 증명하지 않는다.
...
G4E/K2A는 Go 내부의 합성 Kiwoom `LIMIT`/`KRW`/`KRX` 주문 상태 로그를 구현했다. durable client-order idempotency, append-only intent/event, risk verdict 순서, `SUBMIT_UNKNOWN` 재시작 복구, 미확정 command가 있는 계좌의 신규 submit 차단, 알려진 open order의 risk-reducing cancel 허용, fill/cancel/reject replay를 검증한다. 현재 backup v5는 주문 행 hash·metadata·replay와 schema v6의 STRICT/UNIQUE/FK/rowid sequence/insert-only 제약을 source와 restore 후보 모두에서 확인한다. 실제 broker network submit, credentialed mock 호출, public API/UI, 시장가·정정, 체결-원장 reconciliation은 여전히 증명하지 않는다.
...
G6 진입 뒤 첫 로컬 Kubernetes 적합성 검증은 Kind의 Podman provider로 수행한다. 현재 provision된 Kind cluster는 없으며 G6 gate 전에는 manifest를 만들지 않는다.
...
현재 G3 registry는 Python 결과의 result·parameter hash를 Go가 재계산하고 input/config hash 형식·manifest input binding·manifest·gate를 검증해 schema v6 SQLite에 append-only로 등록한다. 원본 bars/config를 받지 않으므로 input/config hash 원본 재계산은 아직 하지 않는다. `paper_candidate`의 명시적 선택과 직전 선택/`no_strategy` 수동 rollback만 제공하고 `no_promotion`은 거절 evidence로 보존한다. backup v5는 registry digest/count/replay를 검증하지만 paper runner, 실제 paper 성능, 자동 성능저하 rollback, 주문 intent의 strategy binding 또는 execution authorization은 아직 증명하지 않는다.
