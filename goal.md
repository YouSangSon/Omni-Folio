...
- 실제 시간 지속 실행은 소유권 관찰, 각 입력의 durable 처리 완료, 종료 후 전체 복구·리소스 반환을 따로 검증한다. 주기적 관찰을 무중단 보증으로, 최신 snapshot 읽기를 누락 없는 durable queue로 간주하지 않는다.
- 중복 이력 검증은 지원 migration 이력과 전체 복구가 실제로 통과한 동일 transaction의 무변경 구간에서만 재사용한다. legacy 조기 반환을 전체 검증으로 간주하지 않으며 독립 호출·새 transaction·변경 후에는 기존 검증을 다시 수행한다.
...
- 이미 접수된 동일 signal의 검증된 재시도만 기존 주문을 서버의 최신 저장 available close까지 복구할 수 있다. 신규 제안의 anchor 제한을 이 예외로 우회하거나 원래 결정에서 새 주문을 만들지 않는다. cached 성과·정책도 현재 selection에 맞는지 기존 검증을 거쳐야 한다.
- 입력 크기 제한만으로 파일 열기의 대기 시간을 제한했다고 간주하지 않는다. FIFO처럼 일반 파일이 아닌 입력은 작성자를 기다리지 않고 거절한다. 로컬 paper 실행은 마지막 durable 성과부터 제안 anchor까지 봉마다 체결·성과·안전정책을 끝낸 뒤 다음 봉으로 진행하고 중간 halt 뒤에는 추가 체결·접수를 하지 않는다. 매 체결의 소유권·회계를 검증하며 해당 봉의 체결량 증가가 없으면 체결 반복만 종료한다. 이미 확정된 성과 뒤의 미완료 정책을 먼저 복구하고 성과 cutoff 뒤에 체결을 끼워 넣지 않는다. 새 성과의 전략 적용 시각 경계를 지키고 기존 성과를 다른 선택에 재귀속하지 않는다. 초기 warmup·이미 확정된 최신 성과 이전의 누락·제공되지 않은 봉을 소급 재생했다고 주장하지 않는다.
