...
- [x] G1 CSV preview → atomic apply → ledger snapshot/receipt, exact cash-flow·split·두 통화 환전 replay, append-only `CASH_VOID`, direct FX observation, cash-only direct-FX valuation와 durable security price observation, schema v11/backup v7 및 legacy v8/v9/v10 owned-copy migration/restore proof
...
- [ ] 원장 후속: holding/PnL/performance valuation, provider FX/price ingestion·source priority·calendar 정책, FX/거래·분할·기업행사 정정, 배당 재투자·국가별 세금 분류와 credentialed broker 체결/현금 reconciliation
