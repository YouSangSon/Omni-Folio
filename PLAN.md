...
- [x] G1 CSV preview → atomic apply → ledger snapshot/receipt, exact cash-flow·split·두 통화 환전 replay, append-only `CASH_VOID`, direct FX observation와 cash-only direct-FX valuation, schema v10와 legacy v8/v9→v10 보존 migration/restore proof
...
- [x] G4E/K2A 내부 합성 주문 상태 로그: LIMIT/KRW/KRX, risk verdict ordering, durable unknown submit, 계좌 단위 신규 submit 차단, 알려진 주문 cancel 허용, 현재 schema v10/backup v6의 주문 로그 복구 증명
...
- [ ] 원장 후속: security price series·holding/PnL/performance valuation, provider FX ingestion·source priority·calendar 정책, FX/거래·분할·기업행사 정정, 배당 재투자·국가별 세금 분류와 credentialed broker 체결/현금 reconciliation
...
- [x] Go/SQLite append-only research evidence·paper-candidate selection registry, stale 선택 차단, 직전 선택/`no_strategy` 수동 rollback, 현재 schema v10/backup v6 복구 증명
...
- [x] G3.6 credential-free paper execution foundation: `paper-signal.v2` 목표 수량을 체결+미완결 BUY에 원자 netting해 Go가 delta OrderIntent를 만들고 K2C risk·공통 주문 상태 머신·결정적 부분/완전 체결·동시 재실행·현재 schema v10/backup v6 복구를 검증하며 Kiwoom transport 진입을 차단
