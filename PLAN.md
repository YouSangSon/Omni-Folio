...
- [x] G1 CSV preview → atomic apply → ledger snapshot/receipt, exact cash-flow·split·두 통화 환전 replay, append-only `CASH_VOID`, direct FX observation, cash-only direct-FX valuation, durable security price observation, internal native-currency holding valuation와 versioned FIFO residual allocation, schema v12/backup v7 및 legacy v8/v9/v10/v11 owned-copy migration/restore proof
...
- [x] G4T credential-free Kiwoom one-shot latest-trade capture: 잘못된 service/client/symbol은 네트워크 전에 거절하고 기존 원장과 같은 `instrument_<symbol>` 식별자를 내부 도출해 G4Q read와 G4S append를 한 번만 연결하며, 직접 쓰기도 대체 instrument ID를 거절하고 provider 실패·같은 슬롯 가격 충돌에도 이전 복구 증거를 보존; durable listing registry·runtime caller·scheduler·freshness·valuation authority는 추가하지 않음
...
- [x] Go/SQLite append-only research evidence·paper-candidate selection registry, stale 선택 차단, 직전 선택/`no_strategy` 수동 rollback, 현재 schema v12/backup v7 복구 증명
...
- [x] G3.6 credential-free paper execution foundation: `paper-signal.v2` 목표 수량을 체결+미완결 BUY에 원자 netting해 Go가 delta OrderIntent를 만들고 K2C risk·공통 주문 상태 머신·결정적 부분/완전 체결·동시 재실행·현재 schema v12/backup v7 복구를 검증하며 Kiwoom transport 진입을 차단
