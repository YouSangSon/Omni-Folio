...
- **Proposal watch**: G3.8G2C `signal_cli --watch`는 고정 연구 후보에서 새로 관찰한 마지막 봉의 제안을 NDJSON으로 출력한다. 파일·DB를 쓰지 않고 주문을 실행하지 않으며, 재시작 중복·중간 snapshot 누락·출력 단절이 가능한 생성 전용 경계다. G3.8G2D `--bundle`은 정확한 두 CSV byte를 보존해 기존 수동 Go `paper-execute -bundle` 검증으로 전달한다. durable 스트림 소비와 지속 실행 lease는 아직 별도 후속이다.
- **Execution renewal**: G3.8G2E는 유효한 현재 owner만 execution/global lease를 한 transaction에서 갱신하며 수동 실행 단계 사이에서 사용한다. 실행 fence는 증가하고 cleanup은 최신 토큰을 따른다. 재활성화·만료 후 재획득·idle heartbeat는 없으며, 갱신 이력이 있는 저장소는 renewal-aware reader가 필요하다. 지속 소비·장기 처리량은 후속 gate다.
