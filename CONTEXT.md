...
- **Proposal watch**: G3.8G2C `signal_cli --watch`는 고정 연구 후보에서 새로 관찰한 마지막 봉의 제안을 NDJSON으로 출력한다. 파일·DB를 쓰지 않고 주문을 실행하지 않으며, 재시작 중복·중간 snapshot 누락·출력 단절이 가능한 생성 전용 경계다. 정확한 원본 byte 전달과 durable 소비는 후속 Go 실행 경계가 담당한다.
