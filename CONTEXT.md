...
- **Paper signal proposal**: `paper-signal-proposal.v1`은 연구 artifact와 실제 CSV hash에 묶인 offline SMA 목표 수량 제안이다. `none/null`은 새 제안 없음이고 `death_cross/"0"`은 청산 목표다. 내부 `paper-signal.v3`나 주문 권한이 아니며 운영 CLI ingress는 아직 연결되지 않았다. `paper-run-loop`는 현재 C3/D/E 성과 평가·안전 정책만 실행한다.
- **Proposal admission**: G3.8G2A 내부 Go use case는 현재 registry·session policy·소유 lease와 저장 봉을 한 transaction에서 확인하고 순수 `strategydomain`이 SMA 제안의 방향을 독립 검증한 뒤 기존 v3 admission으로 연결한다. 생성 시각/만료는 저장 봉의 receipt 시각/+30초이며 호출자가 연장하지 못한다. 운영 CLI·자동 arm·session/bar ingress는 아직 없고 원본 CSV 전체와 저장 series의 연결 증거도 후속 단계다.
