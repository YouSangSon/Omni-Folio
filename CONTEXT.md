...
- **Paper signal proposal**: `paper-signal-proposal.v1`은 연구 artifact와 실제 CSV hash에 묶인 offline SMA 목표 수량 제안이다. `none/null`은 새 제안 없음이고 `death_cross/"0"`은 청산 목표다. 내부 `paper-signal.v3`나 주문 권한이 아니며 Go ingress는 아직 연결되지 않았다. `paper-run-loop`는 현재 C3/D/E 성과 평가·안전 정책만 실행한다.
