...
- **Local paper step**: G3.8G2B의 수동 `paper-execute -arm-paper`는 원본 연구·신규 CSV를 검증하고 기존 eligible 체결→성과 정책→새 접수를 연결한다. 완료된 durable 단계를 지우지 않고 재시도하며 현재 owner/fence만 종료한다. 최초 `paper-init`은 원금만 만들고, `paper-import-bars`는 fixture 관측만 기록한다. 신규 OS signal/stale-owner 실행 matrix와 상시 자동 실행은 미완료다.
