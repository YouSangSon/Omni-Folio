...
- **One-shot Kiwoom latest-trade capture**: G4Q read와 G4S append를 호출 한 번 안에서 순서대로 합성하는 G4T 내부 계약. KRX symbol을 기존 ledger import와 같은 `instrument_<lowercase symbol>`로 내부 변환하고 caller override를 허용하지 않는다. 이는 durable listing ownership이 아니며, capture layer는 재시도하지 않고 provider/persistence 실패 때 이전 known-good series를 유지한다.
...
- G4T capture는 service/client/symbol을 네트워크 전에 검증하고 instrument ID를 기존 ledger import 규칙으로 내부 도출한다. Kiwoom direct write도 다른 ID를 거절하며, capture layer 자체는 provider read나 persistence를 재시도하지 않고 실패를 성공 또는 최신값으로 바꾸지 않는다.
