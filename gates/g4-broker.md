...
7. K2B Kiwoom mock-order submit/query transport and lookup recovery.
8. Toss Securities read-only adapter using the same canonical contracts.
...
- G4H proves only atomic retention of a complete synthetic snapshot and a revision-bound KRX/KRW quantity diff; failed inputs preserve last-known-good and never auto-correct the ledger.
...
- G4E/K2A passes the internal-only Kiwoom synthetic `LIMIT`/`KRW`/`KRX` state log, durable unknown-submit block, risk-reducing cancel, replay/idempotency conflicts and the order portion of current schema/backup v3 recovery checks. No credential or broker request was used and no public route/UI was added.
...
- G4H passes credential-free complete snapshot validation, atomic insert/idempotency, ledger-revision quantity diff, last-known-good retention, insert-only ledger/broker rows and backup v3 broker-state recovery. No credential, actual broker request, scheduler, authoritative freshness, public route/UI or auto-correction was added.
- G4 remains open: real Kiwoom candle/realtime behavior, scheduled credentialed persistence, full broker/ledger reconciliation, physical-device profile, manual VoiceOver/TalkBack, K2B mock transport/unknown correlation/risk/fencing/UI, and every live-order gate are unproven.
