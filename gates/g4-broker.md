...
- G4E/K2A passes the internal-only Kiwoom synthetic `LIMIT`/`KRW`/`KRX` state log, durable unknown-submit block, risk-reducing cancel, replay/idempotency conflicts and the order portion of current schema v6/backup v5 recovery checks. No credential or broker request was used and no public route/UI was added.
