...
- backup candidate를 별도 DB에 복원해 integrity와 golden snapshot을 검증하기 전 active DB를 바꾸지 않는다. 현재 schema v12/backup v7은 cash-void와 FX guard, direct FX 및 security price observation digest/count, 주문 schema/hash/replay, known-good broker snapshot, execution-authority와 risk-reservation digest/count도 함께 검증한다. legacy v5/schema-v8·v9, v6/schema-v10 및 v7/schema-v11 candidate는 원본을 수정하지 않는 owned copy를 v12로 migration한 뒤 검증한다.
...
- `TestSecurityPriceObservation*`와 `TestSchemaMigratesV1ToV12AndReadinessRequiresV12`는 exact identity/as-of, 미래정보 차단, hash replay, DB mutation guard, backup v7 digest/count, v6/schema-v10·v7/schema-v11 owned-copy migration과 약화된 빈 table DDL 거절을 검증한다. 세부 증거는 [`g1h-security-price-observation.md`](g1h-security-price-observation.md)와 [`g4s-kiwoom-durable-price-observation.md`](g4s-kiwoom-durable-price-observation.md)에 있다.
