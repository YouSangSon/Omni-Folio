...
- backup candidate를 별도 DB에 복원해 integrity와 golden snapshot을 검증하기 전 active DB를 바꾸지 않는다. 현재 backup v3는 주문 schema/hash/replay와 known-good broker snapshot digest/count도 함께 검증한다.
...
- `TestGoldenVerticalSliceAndBackupRestore` verifies FIFO values, a transaction-consistent backup candidate, manifest, integrity check, golden snapshot, invalid manifest rejection, and restored DB without replacing the active DB. Current order-aware and broker-aware backup v3 evidence is in [`g4e-kiwoom-synthetic-order-state.md`](g4e-kiwoom-synthetic-order-state.md) and [`g4h-kiwoom-known-good-snapshot.md`](g4h-kiwoom-known-good-snapshot.md).
