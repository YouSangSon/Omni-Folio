...
- [x] G4B1: 명시적으로 전달한 local CSV fixture만 제공하고, 미설정은 `503`, 없는 series는 `404`, 잘못된 query와 port mismatch는 fail-closed한다.
  ...
  EVIDENCE: 2026-08-24 KST PASS. Input is capped at 1 MiB and 500 bars, metadata and timestamps are consistent/strictly ordered, OHLC and volume use exact canonical decimals, and the endpoint pins `source=local_fixture`, `sample=true`, `state=stale` with a non-live issue. The provider-neutral base schema remains separate from this local-fixture subtype.
